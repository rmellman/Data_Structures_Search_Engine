...
public class BTreeImpl<Key extends Comparable<Key>, Value> implements BTree<Key, Value> {
    ...
    public Value get (Key key) {
        ...
        if (entry != null) {
            //did not account for a case where val points to another node
            ...
            if(entry.val instanceof URI){
                Document d = null;
                try {
                    d = (Document) getPM().deserialize(key);
                    entry.val = d;
                } catch (IOException e) {
                    //might have to change this to IOException
    ...
    public Value put (Key key, Value val) {
        ...
        if (alreadyThere != null) {
            //val is a URI so we have to get the doc from the disk
            if(alreadyThere.val instanceof URI){
                try {
                    //Document d = (Document) getPM().deserialize(key);
                    Document toRTN = (Document) getPM().deserialize(key);
                    //getPM().delete(key);
            ...
            //val is a document so we have to replace the old doc with the new doc
            } else if (alreadyThere.val instanceof Document) {
                Document rtn = (Document) alreadyThere.val;
                alreadyThere.val = val;
                return (Value) rtn;
            } else if (alreadyThere.val == null) {
                //val is null, meaning that there was a doc that was deleted (this is an undo) and we have to set val to the new doc
                alreadyThere.val = val;
            }
            return null;
        }
    ...
    public void moveToDisk (Key k) throws Exception {
        ...
        //delete(k);
        //put(k, (Value) k);
        //set k's entry to k (uri) as opposed to the document
        this.get(this.root, k, this.height).val = k;
    }
    @Override
    public void setPersistenceManager (PersistenceManager<Key, Value> pm){
        this.pm = pm;
    }
    private PersistenceManager<Key, Value> getPM(){
        return this.pm;
    }
}
